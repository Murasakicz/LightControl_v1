...
void AsyncFSWebServer::begin(FS* fs) {
    ...
    if (!load_mqtt_config()){ // Try to load mqtt configuration from file system
        defaultMqttConfig(); // Load defaults if any error      
    }
    loadHTTPAuth();
    //WIFI INIT
...
void AsyncFSWebServer::defaultConfig() {
    // DEFAULT CONFIG
    _config.ssid = dc_ssid;
    _config.password = dc_password;
    _config.dhcp = dc_dhcp;
    _config.ip = dc_ip;
    _config.netmask = dc_netmask;
    _config.gateway = dc_gateway;
    _config.dns = dc_dns;
    _config.ntpServerName = dc_ntpServerName;
    _config.updateNTPTimeEvery = dc_updateNTPTimeEvery;
    _config.timezone = dc_timezone;
    _config.daylight = dc_daylight;
    _config.deviceName = dc_deviceName;
...
void AsyncFSWebServer::defaultMqttConfig() {
    // DEFAULT CONFIG
    _mqttConfig.ip = IPAddress (0,0,0,0);
    _mqttConfig.port = 1883;
    _mqttConfig.user = "User";
    _mqttConfig.password = "Password";
    
    _mqttConfig.tsl = false;
    _mqttConfig.firgerprint = ""; 
    
    _mqttConfig.channel0Id = "Chanel0";
    _mqttConfig.channel1Id = "Chanel1";
    _mqttConfig.channel2Id = "Chanel2";
    _mqttConfig.channel3Id = "Chanel3";
...
bool AsyncFSWebServer::load_mqtt_config() {
...
bool AsyncFSWebServer::save_mqtt_config() {
    //flag_config = false;
    DEBUGLOG("Save config\r\n");
    DynamicJsonBuffer jsonBuffer(512);
    //StaticJsonBuffer<1024> jsonBuffer;
    ...
    JsonArray& jsonmqttIP = json.createNestedArray("ip");
    jsonmqttIP.add(_mqttConfig.ip[0]);
    jsonmqttIP.add(_mqttConfig.ip[1]);
    jsonmqttIP.add(_mqttConfig.ip[2]);
    jsonmqttIP.add(_mqttConfig.ip[3]); 
    json["port"] = _mqttConfig.port;
    json["user"] = _mqttConfig.user;
    json["password"] = _mqttConfig.password;

    json["tsl"] = _mqttConfig.tsl;
    json["fingerprint"] = _mqttConfig.firgerprint;
    
    json["channel0Id"] = _mqttConfig.channel0Id;
    json["channel1Id"] = _mqttConfig.channel1Id;
    json["channel2Id"] = _mqttConfig.channel2Id;
    json["channel3Id"] = _mqttConfig.channel3Id;
...
void AsyncFSWebServer::send_mqtt_configuration_values_html(AsyncWebServerRequest *request) {
    ...
    values += "ip_0|" + (String)_mqttConfig.ip[0] + "|input\n";
    values += "ip_1|" + (String)_mqttConfig.ip[1] + "|input\n";
    values += "ip_2|" + (String)_mqttConfig.ip[2] + "|input\n";
    values += "ip_3|" + (String)_mqttConfig.ip[3] + "|input\n";
    values += "port|" + (String)_mqttConfig.port + "|input\n";
    values += "user|" + (String)_mqttConfig.user + "|input\n";
    values += "password|" + (String)_mqttConfig.password + "|input\n";

    values += "tsl|" + (String)(_mqttConfig.tsl ? "checked" : "") + "|chk\n";
    values += "fingerprint|" + (String)_mqttConfig.firgerprint + "|input\n";
    
    values += "ch0_id|" + (String)_mqttConfig.channel0Id + "|input\n";
    values += "ch1_id|" + (String)_mqttConfig.channel1Id + "|input\n";
    values += "ch2_id|" + (String)_mqttConfig.channel2Id + "|input\n";
    values += "ch3_id|" + (String)_mqttConfig.channel3Id + "|input\n";
...
void AsyncFSWebServer::send_mqtt_configuration_html(AsyncWebServerRequest *request) {
    ...
    if (request->args() > 0)  // Save Settings
    {
        ...
        for (uint8_t i = 0; i < request->args(); i++) {
            ...
            if (request->argName(i) == "ip_0") { if (checkRange(request->arg(i)))   _mqttConfig.ip[0] = request->arg(i).toInt(); continue; }
            if (request->argName(i) == "ip_1") { if (checkRange(request->arg(i)))   _mqttConfig.ip[1] = request->arg(i).toInt(); continue; }
            if (request->argName(i) == "ip_2") { if (checkRange(request->arg(i)))   _mqttConfig.ip[2] = request->arg(i).toInt(); continue; }
            if (request->argName(i) == "ip_3") { if (checkRange(request->arg(i)))   _mqttConfig.ip[3] = request->arg(i).toInt(); continue; }
            if (request->argName(i) == "port") { _mqttConfig.port = request->arg(i).toInt();  continue; }
            if (request->argName(i) == "user") { _mqttConfig.user = urldecode(request->arg(i)); continue; }
            if (request->argName(i) == "password") { _mqttConfig.password = urldecode(request->arg(i)); continue; }
            ...
            if (request->argName(i) == "ch0_id") { _mqttConfig.channel0Id = urldecode(request->arg(i)); continue; }
            ...
            if (request->argName(i) == "ch3_id") { _mqttConfig.channel3Id = urldecode(request->arg(i)); continue; }
        }
...
void AsyncFSWebServer::serverInit() {
    ...
    on("/admin/mqttconnectionstate", [this](AsyncWebServerRequest *request) {
        if (!this->checkAuth(request))
            return request->requestAuthentication();
        this->send_mqtt_connection_state_values_html(request);
    });    
...
#define HIDE_SECRET
    ...
    on(MQTT_CONFIG_FILE, HTTP_GET, [this](AsyncWebServerRequest *request) {
    if (!this->checkAuth(request))
      return request->requestAuthentication();
    AsyncWebServerResponse *response = request->beginResponse(403, "text/plain", "Forbidden");
    response->addHeader("Connection", "close");
